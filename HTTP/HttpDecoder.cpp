...
LINE_STATUS HttpDecoder::parse_line(char *buf, int &checked_index, int &read_index) {
...
HTTP_CODE HttpDecoder::parse_request_line(char *buf, CHECK_STATUS &checkStatus) {
    char *tmp = buf;/* 指向数据头部 */
    int cur = 0; /* 解析索引 */
    char* method;/* 请求方法 */
    char* url; /* 请求url */
...
HTTP_CODE HttpDecoder::parse_header(char *buf) {
    char *tmp = buf;
    /* 这一行是最后一行 */
    if ( tmp[0] == '\0' )
        return GET_REQUEST;
...
HTTP_CODE HttpDecoder::parse_all(char *buf, CHECK_STATUS &checkStatus, int &checked_index,
